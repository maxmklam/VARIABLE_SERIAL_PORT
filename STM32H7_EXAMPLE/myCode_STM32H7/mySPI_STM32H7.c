...
int SPI_fd_tx_cpu(struct _DevSPI *pDevSPI)
{
	...
	for(i=0;i<10;i++)
	{
		while(LL_SPI_IsActiveFlag_TXP(pDevSPI->SPIx) != 1U) 
		{;}
...
int SPI_fd_tx_dma(struct _DevSPI *pDevSPI)
{
	...
	LL_DMA_DisableStream(gDevDMAtx.DMAx, pDevSPI->txDmaCh); //Disable DMA Stream

	//LL_DMA_DisableStream(gDevDMAtx.DMAx, pDevSPI->txDmaCh); //Disable DMA Stream
	LL_DMA_SetPeriphAddress(gDevDMAtx.DMAx, pDevSPI->txDmaCh, (u32)&pDevSPI->SPIx->TXDR);
	...
	LL_SPI_SetTransferSize(pDevSPI->SPIx, pDevSPI->xferLen);

	LL_SPI_Enable(pDevSPI->SPIx); //Enable SPI
	LL_DMA_EnableStream(gDevDMAtx.DMAx, pDevSPI->txDmaCh); //Enable DMA Stream
	
	LL_SPI_StartMasterTransfer(pDevSPI->SPIx); //Master transfer start
