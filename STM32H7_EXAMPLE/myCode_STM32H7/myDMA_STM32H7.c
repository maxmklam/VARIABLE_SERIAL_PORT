...
void myDMA_TC_isr(struct _DevDMA *pDevDMA)
{
	...
	if(LL_DMA_IsEnabledStream(pDevDMA->DMAx, pDevDMA->dmaCh) && (LL_DMA_GetMode(pDevDMA->DMAx, pDevDMA->dmaCh) == LL_DMA_MODE_CIRCULAR)) //DMA channel is in circular mode and still enabling, DMA ch will not auto stop but will keep repeating.
	{
		pDevDMA->circularCnt ++;
