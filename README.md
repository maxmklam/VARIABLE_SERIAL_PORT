...
- Huffman-Coding-Like (HCL) compression on transmission: use least bits for most often byte (good for large data transmission)
- Differential Value (DV) for transmission: use leading "1" binary bits for positive differential value, use leading "0" binary bits for negative differential value (good for image/video data transmission)
