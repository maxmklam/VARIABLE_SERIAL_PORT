...
void VSP_LL_init(void)
{
	...
	VSP_1TCLKRATEreg = VSP_1TCLKRATE; //eg, VSP_1TCLKRATEreg=1000000 (1MHz)
	u32 tmrCC1T = 64000000 / VSP_1TCLKRATEreg; //64000000 is timer clock freq
	...
	//----------------------
	u32 DIV = 64000000 / VSP_1TCLKRATE; //64000000 is SPI clock
	...
	Baudrate = Baudrate << SPI_CR1_BR_Pos;
	...
	LL_DMA_DisableChannel(gDevDMArx.DMAx, gDevDMArx.dmaCh);
	...
	LL_TIM_EnableCounter(gDevTIMrx.TIMx);
	LL_DMA_EnableChannel(gDevDMArx.DMAx, gDevDMArx.dmaCh);
...
void VSP_RX_LL_init(void)
{
	...
	LL_DMA_DisableChannel(gDevDMArx.DMAx, gDevDMArx.dmaCh); //reset. 
	
	//for STM32H7 this will trigger DMA TC interrupt when DMA is currently enabled, and can re-enable DMA in isr. But for STM32G0 will not, so need to re-enable DMA here
...
void VSP_TX_LL_cbFunc(enum _DevXferState devXferState, u32 nByteXfered)
{
	...
	switch(devXferState)
	{
		...
		{
			//for test
			testTime32EndDMAtx = myTimebase_tick32();
		 ...
		 #if(VSP_TX_USED_SPI == 1)
			while(LL_SPI_IsActiveFlag_BSY(gDevSPIvsp.SPIx) == 1)
			{;}
			...
			//for test
			u32 testTime32SPI2 = myTimebase_tick32();
			testTimeDiff64SPI2 = myTimebase_diffus32(testTime32SPI2 - testTime32stSPI2); //need 9 us when VSP_1TCLKRATE = 12MHz
		...
		{
			//for test
			//testTime32EndSPI2 = myTimebase_tick32();
		}
		break;
...
void VSP_RX_LL_cbFunc(enum _DevXferState devXferState, u32 nByteXfered)
{
	switch(devXferState)
	{
		...
		case DEVXFERST_TIMOUT: //VSP rx timeout by Tim1Ch2
		{
			...
			//for test
			testTime32EndTIMto = myTimebase_tick32();
		}
		break;
		...
		{			
			...
			//for test
			testTime32EndDMArx = myTimebase_tick32();
		}
		break;
